...
def format(data,city):
    ...
    #print json.dumps(poi_res,ensure_ascii=False).encode("utf-8")
    #print geosearch + q
    if "name" in poi_res:
    ...
    geoencoder_data = get_page(geoencoder+urllib2.quote(geoencoder_str.encode("utf-8")))
...
def test():
    ...
    lines.append(u"北京市海淀区上地东路" )
    lines.append(u"北京市海淀区上地东路1号院" )
    lines.append(u"北京市上地东路鹏寰大厦")
    lines.append(u"北京国贸")
    lines.append(u"北京国贸中心")
    lines.append(u"北京国贸国际会展中心")
    lines.append(u"北京人民大会堂")
    lines.append(u"北京稻香村")
    lines.append(u"(北京)麻雀瓦舍")
    lines.append(u"(北京)国家图书馆")
    lines.append(u"(北京)mao live house")
    try:
        for line in lines:
